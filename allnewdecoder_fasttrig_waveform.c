...
void allnewdecoder_fasttrig_waveform(){
	//char filename[90]="PMT_set3_12ch_Eu152_run01";
	char filename[90]="PMT_set3_12ch_Co60_run02";
...
//	tree->Branch("waveformtime", waveformtime,"waveformtime[waveform_length]/F");
	...
	tree->Branch("FT_time", FT_time, "FT_time[2]/F");
...
//////////////////////
	...
	uint64_t ient=0;
	for (pqrjd_flag[0]=0; ient<numevent/12/2; ){
		...
		if(event_flag==1){
			...
			for (int i=0; i<NHitCh; i++){
				...
				waveformcopy(waveform[i],pulse[i]->waveform);
...
*/			tree->Fill();
			...
			NHitCh=0; FT_time[0]=0.; FT_time[1]=0.;
			event_flag=0;
		}

		if(fread_flag && pulseBuffer->Channel<NOpCh){
			EventNumber=pulseBuffer->EventNumber;
			//pulseBuffer->Analyze(0,200, 200,950, 10, 0.50, samplingrate, 200,600);
		...
		if(fread_flag && pulseBuffer->Channel>=NOpCh){
			if(pulseBuffer->Channel ==8){
				FT_time[pulseBuffer->Group]=pulseBuffer->FastTrig_Analyze(samplingrate, /*float voltage_l=*/1024., /*float voltage_t=*/2048.);
			}
