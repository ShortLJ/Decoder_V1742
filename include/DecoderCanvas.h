class DECODER_CANVAS{
	...
	public:
		...
		DECODER_CANVAS(UInt_t _NOpGr, UInt_t _NOpCh){
			NOpGr=_NOpGr;
			NOpCh=_NOpCh;
...
void DECODER_CANVAS::Construct(){
	Canvas_Decoding = new TCanvas*[NOpGr];
	for (int igr=0; igr<NOpGr; igr++){
		Canvas_Decoding[igr] = new TCanvas(Form("Decoding_gr%d",igr),Form("Decoding_gr%d",igr),1800, 1000);
...
void DECODER_CANVAS::Modified_Update(char flag){
	for (int ich=0; ich<NOpCh; ich++){
		if(flag & 0x01){
			for (int igr=0; igr<NOpGr; igr++){
				Canvas_Decoding[igr]->cd(1+ich+0*NOpCh); gPad->Modified(); gPad->Update();
				Canvas_Decoding[igr]->cd(1+ich+1*NOpCh); gPad->Modified(); gPad->Update();
...
class DECODER_PRIMARY_H1{
	...
	public:
		...
		DECODER_PRIMARY_H1(UInt_t _igroup, UInt_t _NOpCh, int QDCrange, int waveform_length, float sampletime){
			igr=_igroup;
			NOpCh=_NOpCh;
			rPH = new TH1D*[NOpCh];		QDC = new TH1D*[NOpCh];	TDC = new TH1D*[NOpCh];
			for (int ich=0; ich<NOpCh; ich++){
				rPH[ich] = new TH1D(Form("h1_rPH[%d]_gr%d",ich, igr),Form("h1_rPH[%d]_gr%d;rPH;count/rPH",ich, igr), 4096, 0, 4096);
				QDC[ich] = new TH1D(Form("h1_QDC[%d]_gr%d",ich, igr),Form("h1_QDC[%d]_gr%d;QDC;count/%dQDC", ich, igr, QDCrange/2000), 2000, 0, QDCrange);
				TDC[ich] = new TH1D(Form("h1_TDC[%d]_gr%d",ich, igr),Form("h1_TDC[%d]_gr%d;TDC;count/%.1fns",ich, igr, sampletime), waveform_length, 0, waveform_length*sampletime);
			}
		}
		~DECODER_PRIMARY_H1(){
			for (int ich=0; ich<NOpCh; ich++) delete rPH[ich];
			for (int ich=0; ich<NOpCh; ich++) delete QDC[ich];
			for (int ich=0; ich<NOpCh; ich++) delete TDC[ich];
			delete[] rPH;
			delete[] QDC;
			delete[] TDC;
...
class DECODER_QDC_H1_COIN{
	...
	public:
		...
		~DECODER_QDC_H1_COIN(){
			for (int jch=0; jch<NOpCh; jch++) delete coin[jch];
			delete[] coin;
		}
};
...
class DECODER_QDC_H2{
	...
	public:
		...
		~DECODER_QDC_H2(){
			for (int jch=0; jch<NOpCh; jch++) delete yaxis[jch];
			delete[] yaxis;
		}
};
...
class DECODER_TDCDIF_H1{
	...
	public:
		...
		~DECODER_TDCDIF_H1(){
			for (int jch=0; jch<NOpCh; jch++) delete startch[jch];
